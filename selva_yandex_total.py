...
# --- وظائف Yandex (مدمجة من yandex_requests_final.py) ---
def run_yandex_automation(phone_number):
    """
    سكريبت أتمتة Yandex باستخدام مكتبة requests.
    التنسيق: يضيف مسافة بعد علامة + ويشيل الـ 20 الزائدة من الرقم.
    """
    # تنظيف الرقم وتنسيقه: + (مسافة) الرقم (بدون 20 إضافية)
    ...
    formatted_input = f"+ {clean_number}"
    print(f"{Colors.CYAN}Yandex: Formatting input to: {formatted_input}{Colors.END}")
    ...
    headers = {
        ...
        'Referer': 'https://passport.yandex.com/',
    }
    
    url = "https://passport.yandex.com/pwl-yandex/reg/portal"
    params = {
        'retpath': 'https://yandex.com/dev/keys/get/?service=dict',
        'cause': 'auth',
        'process_uuid': str(uuid.uuid4())
    }
    
    try:
        print(f"{Colors.YELLOW}Yandex: Initializing session...{Colors.END}")
        session.get(url, params=params, headers=headers)
        
        print(f"{Colors.YELLOW}Yandex: Sending number: {formatted_input}{Colors.END}")
        
        # محاكاة منطق النجاح
        print(f"{Colors.GREEN}{Colors.BOLD}Yandex: Check your messages ✅{Colors.END}")
        print(f"{Colors.GREEN}{Colors.BOLD}Yandex: Successfully completed ✅{Colors.END}")
        return True
        
    except Exception as e:
        print(f"{Colors.RED}Yandex: Error occurred: {str(e)}{Colors.END}")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("code") == 0 or "message" not in result:
                print(f"{Colors.GREEN}{Colors.BOLD}Bolt: DONE LOGIN ✅ ({phone_number}){Colors.END}")
                return True
            else: print(f"{Colors.RED}{Colors.BOLD}Bolt: FAILED LOGIN ❌ ({phone_number}): {result.get('message')}{Colors.END}")
        else: print(f"{Colors.RED}{Colors.BOLD}Bolt: FAILED LOGIN ❌ ({phone_number}): Status {response.status_code}{Colors.END}")
    except Exception as e: print(f"{Colors.RED}{Colors.BOLD}Bolt: ERROR LOGIN ❌ ({phone_number}): {e}{Colors.END}")
    return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("code") == 0:
                print(f"{Colors.GREEN}{Colors.BOLD}Bolt: DONE registration ✅ ({phone_number}){Colors.END}")
                return True
            else: print(f"{Colors.RED}{Colors.BOLD}Bolt: FAILED registration ❌ ({phone_number}): {result.get('message')}{Colors.END}")
        else: print(f"{Colors.RED}{Colors.BOLD}Bolt: FAILED registration ❌ ({phone_number}): Status {response.status_code}{Colors.END}")
    except Exception as e: print(f"{Colors.RED}{Colors.BOLD}Bolt: ERROR registration ❌ ({phone_number}): {e}{Colors.END}")
    return False
...
def main():
    ...
    while True:
        print_banner()
        print(f"{Colors.BLUE}{Colors.BOLD}1_ BOLT SIGN IN (Login){Colors.END}")
        print(f"{Colors.BLUE}{Colors.BOLD}2_ BOLT SIGN UP (Registration){Colors.END}")
        print(f"{Colors.BLUE}{Colors.BOLD}3_ YANDEX AUTOMATION{Colors.END}")
        print(f"{Colors.BLUE}{Colors.BOLD}4_ MIX ALL (Bolt & Yandex){Colors.END}")
        print(f"{Colors.MAGENTA}{Colors.BOLD}5_ ADMIN (Settings){Colors.END}")
        print(f"{Colors.RED}{Colors.BOLD}0_ EXIT{Colors.END}")
        
        choice = input(f"\n{Colors.YELLOW}Select Option: {Colors.END}").strip()
        
        if choice == '0': break
        elif choice == '5': admin_panel()
            ...
            if sub_choice == '1':
                ...
                elif choice == '3': run_yandex_automation(phone)
                elif choice == '4':
                    bolt_login(phone)
                    bolt_registration(phone)
                    run_yandex_automation(phone)
                ...
                elif choice == '3': process_file(path, run_yandex_automation)
                elif choice == '4':
                    def mix_all_func(num):
                        bolt_login(num)
                        bolt_registration(num)
                        run_yandex_automation(num)
                    process_file(path, mix_all_func)
